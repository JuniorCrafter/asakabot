...
func main() {
	...
	for update := range updates {
			...
			default:
				...
				if state == "ADMIN_ADD_OP" {
					...
					if len(parts) == 2 {
						...
						bot.Send(tgbotapi.NewMessage(userID, "❌ Неверный формат."))
					}
					repository.UpdateUserState(db, userID, "ADMIN_MAIN")
		...
		case text == i18n.Get(lang, "btn_contacts"):
			contactsText := "📞 *Наши контактные номера:*\n\n" +
				"🏢 Основной: +998 71 123 45 67\n" +
				"🎧 Поддержка: +998 90 987 65 43\n" +
				"💼 Для юр. лиц: +998 99 111 22 33\n\n" +
				"_Нажмите на номер, чтобы позвонить._"
		...
		case text == i18n.Get(lang, "btn_about"):
			...
			infoMsg.ReplyMarkup = tgbotapi.NewInlineKeyboardMarkup(
				tgbotapi.NewInlineKeyboardRow(
					tgbotapi.NewInlineKeyboardButtonURL("📢 Наш Telegram канал", "https://t.me/asakabank"),
				),
			)
			bot.Send(infoMsg)

			// Отправка геолокации
			locMsg := tgbotapi.NewLocation(update.Message.Chat.ID, 41.042300, 69.356400)
			bot.Send(locMsg)
